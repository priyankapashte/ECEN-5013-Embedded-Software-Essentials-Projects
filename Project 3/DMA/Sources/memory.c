...

{
	...
	if((src != NULL) && (dst != NULL) && (length > MEMORY_NUM_ZERO))       	/* Check for NULL pointer of src and dst and length >0 */
	{
		if(src != dst)                                                   /* Checking if src and dst address are not same */
		{
			...
			{
				for (uint8_t i = MEMORY_NUM_ZERO; i != length; i++) 
				{
...
int8_t  my_memset(uint8_t * src, uint32_t length, uint8_t value)
{
	...
	{	
		...
		for(uint8_t increment=MEMORY_NUM_ZERO; increment < length; increment++)
		{
...
{	
	...
	{		 
		...
		for(int8_t loop=MEMORY_NUM_ZERO; loop < length; loop++)            /* Replacing the value with zero till loop matches length */
		{
			*src =  MEMORY_NUM_ZERO;
			src++;
		}
